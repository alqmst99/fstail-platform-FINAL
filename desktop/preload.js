...
// Alias compatible con el QuickBidCard y Dashboard
contextBridge.exposeInMainWorld('electronAPI', {
  notify: (payload) => ipcRenderer.invoke('notify', payload),
  onNavigateToApplication: (callback) => {
    const handler = (_e, applicationId) => callback(applicationId);
    ipcRenderer.on('navigate-to-application', handler);
    return () => ipcRenderer.removeListener('navigate-to-application', handler);
  },
});
