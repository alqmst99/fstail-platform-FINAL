...
function registerIpcHandlers() {
  ...
  // ── OS Notifications (with deep-link to application) ───────────────
  ipcMain.handle('notification:show', (_e, { title, body, applicationId }) => {
    ...
    notification.on('click', () => {
      if (mainWindow) {
        ...
        mainWindow.focus();
        if (applicationId) {
          mainWindow.webContents.send('navigate-to-application', applicationId);
        }
      }
    });
    notification.show();
  ...
  // Alias usado por QuickBidCard (electronAPI.notify)
  ipcMain.handle('notify', (_e, payload) => {
    const { Notification } = require('electron');
    if (!Notification.isSupported()) return;
    const notification = new Notification({
      title: payload.title || 'FSTail',
      body: payload.body || '',
      silent: false,
    });
    notification.on('click', () => {
      if (mainWindow) {
        ...
        if (payload.applicationId) {
          mainWindow.webContents.send('navigate-to-application', payload.applicationId);
        }
      }
    });
    notification.show();
